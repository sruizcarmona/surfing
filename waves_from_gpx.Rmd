...
session$totaltime <- as.numeric(gpx %>% summarise(totaltime=as.numeric(difftime(last(time),first(time),units = "mins"))) )
session$waitingtime_waves <- session$totaltime / session$nwaves
# time per wave
session$maxtime_singlewave <- max(wave %>% 
  filter(wave.n != 0) %>% 
  group_by(wave.n) %>% 
  summarise(wave.time = last(cum_s)-first(cum_s), .groups='drop') %>% 
  select(wave.time))
session$totaltime_allwaves <- sum(wave %>% 
  filter(wave.n != 0) %>% 
  group_by(wave.n) %>% 
  summarise(wave.time = last(cum_s)-first(cum_s), .groups='drop') %>% 
  select(wave.time))
session$averagetime_allwaves <- round(session$totaltime_allwaves / session$nwaves,1)
# distance per wave
session$maxdist_singlewave <- max(wave %>% 
  filter(wave.n != 0) %>% 
  group_by(wave.n) %>% 
  summarise(wave.x= last(cum_distance)-first(cum_distance), .groups='drop') %>% 
  select(wave.x))
session$totaldist_allwaves <- sum(wave %>% 
  filter(wave.n != 0) %>% 
  group_by(wave.n) %>% 
  summarise(wave.x = last(cum_distance)-first(cum_distance), .groups='drop') %>% 
  select(wave.x))
session$averagedist_allwaves <- round(session$totaldist_allwaves / session$nwaves,1)
session
```
